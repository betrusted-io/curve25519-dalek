...
pub mod utra {
    pub mod engine {
        ...
        pub const WINDOW: Register = Register::new(0, 0xf);
        pub const WINDOW_WINDOW: Field = Field::new(4, 0, WINDOW);

        pub const MPSTART: Register = Register::new(1, 0x3ff);
        pub const MPSTART_MPSTART: Field = Field::new(10, 0, MPSTART);

        pub const MPLEN: Register = Register::new(2, 0x3ff);
        pub const MPLEN_MPLEN: Field = Field::new(10, 0, MPLEN);

        pub const CONTROL: Register = Register::new(3, 0x1);
        pub const CONTROL_GO: Field = Field::new(1, 0, CONTROL);

        pub const MPRESUME: Register = Register::new(4, 0x3ff);
        pub const MPRESUME_MPRESUME: Field = Field::new(10, 0, MPRESUME);

        pub const POWER: Register = Register::new(5, 0x3);
        pub const POWER_ON: Field = Field::new(1, 0, POWER);
        pub const POWER_PAUSE_REQ: Field = Field::new(1, 1, POWER);

        pub const STATUS: Register = Register::new(6, 0xfff);
        pub const STATUS_RUNNING: Field = Field::new(1, 0, STATUS);
        pub const STATUS_MPC: Field = Field::new(10, 1, STATUS);
        pub const STATUS_PAUSE_GNT: Field = Field::new(1, 11, STATUS);

        pub const EV_STATUS: Register = Register::new(7, 0x3);
        pub const EV_STATUS_FINISHED: Field = Field::new(1, 0, EV_STATUS);
        pub const EV_STATUS_ILLEGAL_OPCODE: Field = Field::new(1, 1, EV_STATUS);

        pub const EV_PENDING: Register = Register::new(8, 0x3);
        pub const EV_PENDING_FINISHED: Field = Field::new(1, 0, EV_PENDING);
        pub const EV_PENDING_ILLEGAL_OPCODE: Field = Field::new(1, 1, EV_PENDING);

        pub const EV_ENABLE: Register = Register::new(9, 0x3);
        pub const EV_ENABLE_FINISHED: Field = Field::new(1, 0, EV_ENABLE);
        pub const EV_ENABLE_ILLEGAL_OPCODE: Field = Field::new(1, 1, EV_ENABLE);

        pub const INSTRUCTION: Register = Register::new(10, 0xffffffff);
        pub const INSTRUCTION_OPCODE: Field = Field::new(6, 0, INSTRUCTION);
        pub const INSTRUCTION_RA: Field = Field::new(5, 6, INSTRUCTION);
        pub const INSTRUCTION_CA: Field = Field::new(1, 11, INSTRUCTION);
        pub const INSTRUCTION_RB: Field = Field::new(5, 12, INSTRUCTION);
        pub const INSTRUCTION_CB: Field = Field::new(1, 17, INSTRUCTION);
        pub const INSTRUCTION_WD: Field = Field::new(5, 18, INSTRUCTION);
        pub const INSTRUCTION_IMMEDIATE: Field = Field::new(9, 23, INSTRUCTION);

        pub const ENGINE_IRQ: usize = 18;
        pub const HW_ENGINE_BASE :   usize = 0xf000_1000; // this is manually copied from soc.svd in the workspace target directory
    }
}
...
#[cfg(feature = "pac_test")]
fn copy_to_rf(bytes: [u8; 32], register: usize, rf: &mut [u32]) {
    //let report_ptr: *mut u32 = 0xf000_0004 as *mut u32;
    let rf_ptr: *mut u32 = (HW_ENGINE_MEM + 0x1_0000) as *mut u32;
    for (i, byte) in bytes.chunks_exact(4).enumerate() {
        let word = u32::from_le_bytes(byte.try_into().unwrap());
        //unsafe{report_ptr.write_volatile(word)};
        unsafe{rf_ptr.add(i + register*8).write_volatile(word);}
    }
    /*
...
#[cfg(feature = "pac_test")]
fn copy_from_rf(register: usize, rf: &[u32]) -> [u8; 32] {
...
impl<'a, 'b> Mul<&'b Scalar> for &'a MontgomeryPoint {
    ...
    fn mul(self, scalar: &'b Scalar) -> MontgomeryPoint {
        ...
        let ucode = unsafe{core::slice::from_raw_parts_mut(HW_ENGINE_MEM as *mut u32, 1024)};
        let rf = unsafe{core::slice::from_raw_parts_mut((HW_ENGINE_MEM + 0x1_0000) as *mut u32, 256)};

        // load microcode
        for (&src, dst) in mcode.iter().zip(ucode.iter_mut()) {
            *dst = src as u32;
        }
        // set core registers
        engine_csr.wfo(utra::engine::WINDOW_WINDOW, 0 as u32);
        engine_csr.wfo(utra::engine::MPSTART_MPSTART, 0);
        engine_csr.wfo(utra::engine::MPLEN_MPLEN, mcode.len() as u32);

        copy_to_rf(x0.U.to_bytes(), 25, rf);
        copy_to_rf(x0.W.to_bytes(), 26, rf);
        copy_to_rf(x1.U.to_bytes(), 27, rf);
        copy_to_rf(x1.W.to_bytes(), 28, rf);
        copy_to_rf(affine_u.to_bytes(), 24, rf);
        copy_to_rf(scalar.bytes, 31, rf);
        ...
        // start the run
        engine_csr.wfo(utra::engine::CONTROL_GO, 1);
        let report_ptr: *mut u32 = 0xf000_0004 as *mut u32;
        loop {
            let status = engine_csr.r(utra::engine::STATUS);
            unsafe{report_ptr.write_volatile(status);}
            if status & 1 == 0 {
                break;
            }
        }
        // return the result
        MontgomeryPoint(copy_from_rf(31, &rf))
    }
    #[cfg(not(feature = "pac_test"))]
    fn mul(self, scalar: &'b Scalar) -> MontgomeryPoint {
        // Algorithm 8 of Costello-Smith 2017
        #[cfg(all(not(test),feature="betrusted"))] // due to issue https://github.com/rust-lang/rust/issues/59168, you will have to manually comment this out when running a test on the full system and not just this crate.
